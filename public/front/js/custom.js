...
$(document).ready(function() {
    ...
    $('input[name=address_id]').bind('change', function () {
        var shipping_charges = $(this).attr('shipping_charges'); // Get shipping charges from the selected address
        var total_price = $(this).attr('total_price'); // Get total price
        var coupon_amount = $(this).attr('coupon_amount'); // Get coupon amount
    
        if (coupon_amount == '') {
            coupon_amount = 0; // Set coupon amount to 0 if not available
        }
    
        // Event listener for payment method change
        $('input[name=payment_gateway]').unbind('change').bind('change', function () {
            var payment_method = $(this).val(); // Get the selected payment method
    
            if (payment_method === 'COD') {
                // If Cash on Delivery is selected, display the shipping charges
            ...
            } else if (payment_method === 'Pick-Up') {
                // If Pick-Up is selected, set shipping charges to 0
                $('.shipping_charges').html('₱0');
    
                // Calculate and display the grand total
                var grand_total = parseInt(total_price) - parseInt(coupon_amount); // No shipping charges for Pick-Up
                $('.grand_total').html('₱' + grand_total);
            }
        });
    
        // Trigger a change event on the payment method to set initial values
