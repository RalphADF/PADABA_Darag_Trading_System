...
@section('content')
    ...
    <script>
        $(function () {
            var ordersByTime = @json($ordersByTime);
            var labels = [];
            var totals = [];
            ...
            ordersByTime.forEach(function (entry) {
                var month = monthNames[entry.month - 1];
                labels.push(`${month} ${entry.year}`);
            ...
            function calculateWMA(data, windowSize) {
                var weights = Array.from({ length: windowSize }, (_, i) => i + 1);
                var wma = [];
            ...
            function predictFutureSales(wmaData, futureMonths) {
                var futureForecasts = [];
                var recentTrend = wmaData.slice(-2); // Last two WMA values for trend calculation
    
                var trend = recentTrend.length === 2 ? (recentTrend[1] - recentTrend[0]) : 0;
    
                for (var i = 0; i < futureMonths; i++) {
                    var nextValue = (wmaData[wmaData.length - 1] || 0) + trend;
                    futureForecasts.push(nextValue);
                    wmaData.push(nextValue);
                }
            ...
            var futureMonths = 3;
            var futurePredictions = predictFutureSales(forecast, futureMonths);
    
            function getNextMonthLabel(lastLabel, monthsAhead) {
                var parts = lastLabel.split(" ");
                var lastMonth = parts[0];
                var lastYear = parseInt(parts[1]);
    
                var lastMonthIndex = monthNames.indexOf(lastMonth);
                ...
                for (var i = 0; i < monthsAhead; i++) {
                    ...
                    labels.push(`${monthNames[lastMonthIndex]} ${lastYear}`);
                }
            }
    
            getNextMonthLabel(labels[labels.length - 1], futureMonths);
    ...
    <script>
        $(function () {
            ...
            // Function to perform Seasonal ARIMA-based forecasting
            function seasonalARIMAForecast(data, futureMonths) {
                var n = data.length;
                var p = 1, d = 1, q = 1; // ARIMA(p,d,q) parameters (adjustable)
                var seasonalP = 1, seasonalD = 1, seasonalQ = 1, seasonalPeriod = 12; // Seasonal components

                // Apply differencing (d)
                var differenced = [];
                for (var i = d; i < n; i++) {
                    differenced.push(data[i] - data[i - d]);
                }

                // Fit ARIMA model using basic mean reversion approach (simplified for JS)
                var mean = jStat.mean(differenced);
                var forecast = data.slice();
                for (var i = 0; i < futureMonths; i++) {
                    var nextValue = forecast[forecast.length - 1] + mean; // Using naive mean reversion
            ...
            // Forecast the next 3 months using Seasonal ARIMA
            var futureMonths = 3;
            var forecast = seasonalARIMAForecast(totals, futureMonths);
            ...
            var data = {
                ...
                datasets: [
                    ...
                    {
                        label: 'Forecasted Sales (Seasonal ARIMA)',
                        data: forecast, // Forecast data using Seasonal ARIMA
                        borderColor: 'rgba(255, 99, 132, 1)',
                        borderWidth: 2,
                        fill: false,
                        borderDash: [5, 5], // Dash pattern for the forecast line
